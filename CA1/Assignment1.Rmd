...
For this we will have to iteratively adjust  a,b,c,d to meet both conditions.

### Code:
```{r echo=FALSE}
# Function to compute odds ratio and confidence interval
compute_odds_ratio <- function(a, b, c, d) {
  OR <- (a * d) / (b * c)
  ln_OR <- log(OR)
  SE <- sqrt(1/a + 1/b + 1/c + 1/d)
  CI_lower <- exp(ln_OR - 1.96 * SE)
  CI_upper <- exp(ln_OR + 1.96 * SE)
...
# Initialize table counts with large values
a <- 1000
b <- 1000
c <- 1001
d <- 1001

# Compute initial odds ratio and confidence interval
result <- compute_odds_ratio(a, b, c, d)

# Adjust counts iteratively to meet conditions
while (result$OR < 0.99 || result$OR > 1.01 || result$CI[1] <= 1 || result$CI[2] >= 1) {
  # Increment values to adjust odds ratio closer to 1
  a <- a + 1
  c <- c + 1
  
  # Recalculate odds ratio and confidence interval
  result <- compute_odds_ratio(a, b, c, d)
}

# Print final table and results
cat("Final Table:\n")
cat("a =", a, "b =", b, "c =", c, "d =", d, "\n")
cat("Odds Ratio =", result$OR, "\n")
cat("95% Confidence Interval =", result$CI, "\n")
```
### Results and Interpretation:
